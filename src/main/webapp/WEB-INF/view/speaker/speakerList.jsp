...
<html lang="zh-CN">                                                                                                                        
  ...
  <div class="jumbotron"  >
    <h2 style="padding-left: 400px;">主讲人列表--主讲人管理</h2>
            ...
            <div>
            	 <label for="inputEmail3" class="col-sm-2 control-label"></label>
				    ...
				    <a class="btn btn-primary" href="${pageContext.request.contextPath }/adminSpeaker/speaker_addSpeaker.action">添加主讲人</a>
				   
  			        <div style="float: right ; margin-right: 600px;">
  			        <form action="${pageContext.request.contextPath }/adminSpeaker/speaker_speakerList.action">
  			        	<span>名称</span>
    ...
    <table class="table table-hover  table-condensed"    style="width: 1000px; ">
      ...
      <tbody>
      <c:forEach items="${pageUtil.rows }" varStatus="status"  var="speaker">
        <tr>
          <th scope="row">${ status.count + (pageUtil.page-1)*5}</th>
          <td>${speaker.speakerName }</td>
          <td>${speaker.speakerJob }</td>
          <td>${speaker.speakerDescr }</td>
          <td><a class="glyphicon glyphicon-pencil"     
           href="${pageContext.request.contextPath }/adminSpeaker/speaker_updateSpeaker01.action?id=${speaker.id}"></a></td>
        ...
        <%--   onclick='return(confirm("确定删除?"));'  href="${pageContext.request.contextPath }/speaker/deleteSpeaker.action?id=${speaker.id}" --%>
                    onclick="delcfm('${pageContext.request.contextPath }/adminSpeaker/speaker_deleteSpeaker.action?id=${speaker.id}')" ></a></td>
        </tr>
        </c:forEach>
      </tbody>
    </table>
  ...
  <div class="modal fade" id="delcfmModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div>
       <label for="inputEmail3" class="col-sm-2 control-label"></label>
    <page:page url="${pageContext.request.contextPath }/adminSpeaker/speaker_speakerList.action"></page:page>
     </div>
  </div>
