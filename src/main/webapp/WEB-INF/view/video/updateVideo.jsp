...
<html lang="zh-CN">                                                                                                                        
  ...
  <form class="form-horizontal"  action="${pageContext.request.contextPath }/adminVideo/video_updateVideo02.action"  >
        ...
        <div class="col-sm-10" style="width: 600px;">
        	<select class="form-control" id="inputPassword3" name="speakerId"> 
        	    ...
        	    <c:forEach items="${speaker}" var="speaker">
  			       <option value="${speaker.id }"  ${video.speaker.id == speaker.id ? "selected":""}>${speaker.speakerName}</option>
  			    </c:forEach>
        	</select>
        </div>
        </div>
        	...
        	<select class="form-control" id="inputPassword3"  name="courseId"> 
        	  <option value="">请选择课程</option>
        	   <c:forEach  items="${course }" var="course">
  			       <option value="${course.id }"    ${video.course.id == course.id ? "selected":""}>${course.courseName }</option>
  			   </c:forEach>
        	</select>
        </div>
        </div>
